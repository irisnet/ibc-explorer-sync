...
func ParseBlockAndTxs(b int64, client *pool.Client) (*models.Block, []*models.Tx, error) {
	...
	if len(block.Block.Txs) > 0 {
		for i, v := range block.Block.Txs {
			if !includeIbcTxs(v, blockDoc.Height) {
				continue
			}
...
// Parse events when transaction log is empty for successful transactions
...
// Parse txResult.events and keep only IBC-related sub-events
var (
