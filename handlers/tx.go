...
func InitRouter(conf *config.Config) {
	...
	// IBC zone - filter message types for IBC
			...
			if len(_filterMap) > 0 {
				for id, one := range docTx.EventsNew {
					if one.MsgIndex == uint32(msgIndex) {
						docTx.EventsNew[id].Events = []models.Event{}
						break
					}
				}
...
// For some chains like Osmosis, when transaction logs are empty in success state, parse from events
...
func parseABCIEvents(txResult *ctypes.ResultTx) (map[uint32]models.EventNew, []models.EventNew) {
	...
	// Store according to msg index order
