...
func parseTx(txBytes types.Tx, txResult *ctypes.ResultTx, block *types.Block, index int) (models.Tx, error) {
	...
	for msgIndex, v := range msgs {
		...
		// If this is ICA cross-chain data, do not save events and msg content
...
// Handle parsing from events when transaction log is empty for successful transactions on some chains
...
// Parse from txResult.events and keep only IBC-related sub-events
var (
...
func parseABCIEvents(txResult *ctypes.ResultTx) (map[uint32]models.EventNew, []models.EventNew) {
	...
	for i := range txResult.TxResult.Events {
		...
		for index := lengAttr - 1; index >= 0; index-- {
			if string(txResult.TxResult.Events[i].Attributes[index].Key) == constant.IbcTxEventAttriKeyMsgIndex {
				msgIndex = string(txResult.TxResult.Events[i].Attributes[index].Value)
				break
			}
		}
		if msgIndex != "" {
